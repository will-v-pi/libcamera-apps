...
void Output::OutputReady(void *mem, size_t size, int64_t timestamp_us, bool keyframe, libcamera::ControlList metadata)
{
	...
	// Save metadata to file, if that was requested
...
void Output::metadataReady(const libcamera::ControlList &metadata)
{
	if (options_->metadata.empty())
		return;
